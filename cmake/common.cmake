...
macro(handle_data_files)
	...
	cmake_parse_arguments("${_PREFIX}" "${_OPTIONS}" "${_ONE_VALUE_ARGS}" "${_MULTI_VALUE_ARGS}" "${ARGN}")
	...
	if("${COPY_OUTPUT}")
		foreach(_FILE ${${_PREFIX}_FILES})
			add_custom_command(
				TARGET "${PROJECT_NAME}"
				POST_BUILD
				COMMAND "${CMAKE_COMMAND}" -E copy_if_different "${_FILE}" "${Fallout4Path}/Data/${${_PREFIX}_COPY_DESTINATION}"
				VERBATIM
			)
		endforeach()
	endif()
